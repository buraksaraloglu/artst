...
import { APP_DOMAIN, cn, fetcher } from "@artst/utils";
...
export function NavMobile() {
	const { domain = "artst.io" } = useParams() as { domain: string };
	...
	const { data: session, isLoading } = useSWR(
		domain === "artst.io" && "/api/auth/session",
		fetcher,
		{
				...
				<ul className="grid divide-y divide-gray-200">
					<li className="py-3">
						...
						>
							...
							<ChevronDown
								className={cn("h-5 w-5 text-gray-500 transition-all", openFeatures && "rotate-180")}
							/>
						</button>
						{openFeatures && (
							<div className="grid gap-4 overflow-hidden py-4">
								{FEATURES_LIST.map(({ slug, icon: Icon, shortTitle }) => (
									<Link
										key={slug}
										href={domain === "artst.io" ? `/${slug}` : `https://artst.io/${slug}`}
					...
					{session && Object.keys(session).length > 0 ? (
						<li className="py-3">
							<Link href={APP_DOMAIN} className="flex w-full font-semibold capitalize">
								Dashboard
							</Link>
						</li>
					) : (
						<>
							<li className="py-3">
								<Link href={`${APP_DOMAIN}/login`} className="flex w-full font-semibold capitalize">
									Log in
								</Link>
							</li>
