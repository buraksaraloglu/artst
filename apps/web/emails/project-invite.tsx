				...
				<Body className="mx-auto my-auto bg-white font-sans">
					<Container className="mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5">
						<Section className="mt-8">
							<Img src={ARTST_LOGO} width="40" height="40" alt={appName} className="mx-auto my-0" />
						</Section>
						...
						{projectUser && projectUserEmail ? (
							<Text className="text-sm leading-6 text-black">
								...
								<Link className="text-blue-600 no-underline" href={`mailto:${projectUserEmail}`}>
									{projectUserEmail}
								</Link>
								) has invited you to join the <strong>{projectName}</strong> project on {appName}!
							</Text>
						) : (
							<Text className="text-sm leading-6 text-black">
								You have been invited to join the <strong>{projectName}</strong> project on{" "}
								{appName}!
							</Text>
						)}
