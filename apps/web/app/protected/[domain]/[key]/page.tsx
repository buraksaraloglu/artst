...
import { Background, Logo } from "@artst/ui";
import { constructMetadata, isArtstDomain } from "@artst/utils";
...
const image = "https://artst.io/_static/password-protected.png";
...
export async function generateMetadata({ params }: { params: { domain: string; key: string } }) {
	...
	return constructMetadata({
		title: isArtstDomain(domain) || link.project?.plan === "free" ? `${title} - artst.io` : title,
		description,
		image,
		...(!isArtstDomain(domain) &&
		...
		<main className="flex h-screen w-screen items-center justify-center">
			...
			<div className="w-full max-w-md overflow-hidden rounded-2xl border border-gray-100 shadow-xl">
				<div className="flex flex-col items-center justify-center space-y-3 border-b border-gray-200 bg-white px-4 py-6 pt-8 text-center sm:px-16">
					{!isArtstDomain(domain) && link.project?.plan !== "free" && link.project?.logo ? (
						<img
						...
						<a href="https://artst.io" target="_blank" rel="noreferrer">
							<Logo />
						</a>
					)}
