...
import { LOCALHOST_IP, getSearchParams } from "@artst/utils";
...
export const GET = async (req: NextRequest, { params }: { params: Record<string, string> }) => {
	...
	// demo link (artst.io/try)
	if (domain === "artst.io" && key === "try") {
		...
		if (process.env.NODE_ENV !== "development") {
			...
			const { success } = await ratelimit(15, endpoint === "clicks" ? "10 s" : "1 h").limit(
				`${ip}:${domain}:${key}:${endpoint}`,
			);
