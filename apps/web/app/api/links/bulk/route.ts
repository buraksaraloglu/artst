...
export const POST = withAuth(
	async ({ req, headers, session, project }) => {
		...
		const processedLinks = await Promise.all(
			links.map(async (link) => processLink({ payload: link, project, session, bulk: true })),
		);

		const validLinks = processedLinks.filter(({ error }) => !error).map(({ link }) => link);
		...
		const validLinksResponse = validLinks.length > 0 ? await bulkCreateLinks(validLinks) : [];
