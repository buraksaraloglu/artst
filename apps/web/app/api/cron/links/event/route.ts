...
import { APP_DOMAIN_WITH_NGROK, GOOGLE_FAVICON_URL, getApexDomain, log } from "@artst/utils";
...
export async function POST(req: Request) {
	...
	if (link.domain === "artst.io") {
		const invalidFavicon = await fetch(`${GOOGLE_FAVICON_URL}${getApexDomain(link.url)}`).then(
			(res) => !res.ok,
		);
	...
	if (type === "create" && link.projectId) {
		...
		const percentage = Math.round((project.linksUsage / project.linksLimit) * 100);
