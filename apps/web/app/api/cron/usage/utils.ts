...
export const updateUsage = async (skip?: number) => {
	...
	const exceedingUsage = projects.filter(({ usage, usageLimit }) => usage > usageLimit);
	...
	await Promise.allSettled(
		billingReset.map(async (project) => {
			...
			if (project.createdAt.getTime() < new Date().getTime() - 30 * 24 * 60 * 60 * 1000) {
				const topLinks =
					project.usage > 0
						? await getStats({
								...
								data
									.slice(0, 5)
									.map(
										({ domain, key, clicks }: { domain: string; key: string; clicks: number }) => ({
	...
	return await qstash.publishJSON({
		url: `${APP_DOMAIN_WITH_NGROK}/api/cron/usage?skip=${skip ? skip + limit : limit}`,
		method: "GET",
	});
};
