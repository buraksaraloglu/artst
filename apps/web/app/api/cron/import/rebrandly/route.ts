...
export async function POST(req: Request) {
	...
	try {
		...
		const rebrandlyApiKey = await redis.get<string>(`import:rebrandly:${projectId}`);
		...
		if (importTags === true) {
			const tagsImported = await redis.get(`import:rebrandly:${projectId}:tags`);
