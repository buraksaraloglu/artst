...
import { FREE_PLAN, getPlanFromPriceId, log } from "@artst/utils";
	...
	if (relevantEvents.has(event.type)) {
		try {
			...
			if (event.type === "customer.subscription.deleted") {
				...
				const projectUsers = project.users.map(({ user }) => user.email as string);
				...
				await Promise.allSettled([
					...
					pipeline.exec(),
					log({
						message: ":cry: Project *`" + project.slug + "`* deleted their subscription",
						type: "cron",
						mention: true,
					}),
					resend.emails.send({
						from: "Burak from Artst.io <burak@artst.io>",
						to: projectUsers,
