...
import { Button, Copy, LoadingSpinner, Tick } from "@artst/ui";
	...
	return (
		<div className="flex flex-col space-y-5">
			...
			{data && (
				<form
					action={(formData) => {
						if (data.proProjectSlugs.length > 0 || data.verifiedDomains.length > 0) {
							if (
								!confirm(
		...
		<div className="relative flex w-full rounded-md shadow-sm">
			...
			<input
				...
				onPaste={(e) => {
					...
					const pastedText = e.clipboardData.getData("text").replace(/https?:\/\/artst\.io\//, "");
