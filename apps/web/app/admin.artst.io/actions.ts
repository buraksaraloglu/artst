...
import { ARTST_PROJECT_ID, LEGAL_USER_ID, SHORT_DOMAIN, getDomainWithoutWWW } from "@artst/utils";
...
export async function getUserByKey(data: FormData) {
	...
	return {
		...
		proProjectSlugs:
			projects.filter(({ project }) => project.plan === "pro").map(({ project }) => project.slug) ||
			[],
