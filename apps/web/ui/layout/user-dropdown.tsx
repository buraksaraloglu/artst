...
import { Avatar, Badge, IconMenu, LoadingCircle, Popover } from "@artst/ui";
import { HOME_DOMAIN } from "@artst/utils";
				...
				content={
					<div className="flex w-full flex-col space-y-px rounded-md bg-white p-3 sm:w-56">
						...
						>
							<IconMenu
								text="Support"
								icon={openingSupport ? <LoadingCircle /> : <MessageCircle className="h-4 w-4" />}
							/>
						</button>
						<Link
							href="/settings"
						...
						<Link
							...
							target="_blank"
							rel="noopener noreferrer"
							onClick={() => Cookies.set("lastReadChangelog", new Date().toISOString())}
						...
						>
							...
							{unreadChangelogs > 0 && <Badge variant="blue">{unreadChangelogs}</Badge>}
						</Link>
						<button
							className="w-full rounded-md p-2 text-sm transition-all duration-75 hover:bg-gray-100 active:bg-gray-200"
							onClick={() => {
