...
import { Popover, Tick } from "@artst/ui";
import { GOOGLE_FAVICON_URL, SHORT_DOMAIN } from "@artst/utils";
		...
		if (slug && projects && selectedProject) {
			return {
				...
				image:
					...
					`${GOOGLE_FAVICON_URL}${selectedProject?.primaryDomain?.slug || SHORT_DOMAIN}`,
			};
...
function ProjectList({
	...
	return (
		<div className="relative mt-1 max-h-72 w-full space-y-0.5 overflow-auto rounded-md bg-white p-2 text-base sm:w-60 sm:text-sm sm:shadow-lg">
			<div className="p-2 text-xs text-gray-500">My Artists</div>
			{projects.map(({ id, name, slug, logo, primaryDomain, metadata }) => {
				return (
					...
					>
						<img
							src={logo || `${GOOGLE_FAVICON_URL}${primaryDomain?.slug || SHORT_DOMAIN}`}
							alt={id}
			...
			>
				...
				<span className="block truncate">Add a new artist</span>
			</button>
		</div>
	);
}
