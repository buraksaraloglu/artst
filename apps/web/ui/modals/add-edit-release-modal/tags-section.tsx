...
import { LoadingCircle, SimpleTooltipContent, Tooltip } from "@artst/ui";
			...
			<Command ref={commandRef} className="relative" loop>
				<div className="group mt-1 rounded-md border border-gray-300 bg-white px-1 focus-within:border-gray-500 focus-within:ring-1 focus-within:ring-gray-500">
					...
					<div className="flex h-9 px-8">
						{selectedTag ? <TagBadge key={selectedTag.id} {...selectedTag} /> : <CommandInput />}
						{selectedTag ? (
							<button
								onClick={() => {
				...
				{openCommandList && (
					...
					>
						...
						{tags?.map((tag) => (
							...
							>
								<TagBadge {...tag} />
								{selectedTag?.id === tag.id && <Check className="h-5 w-5 text-gray-500" />}
							</Command.Item>
						))}
					</Command.List>
				)}
