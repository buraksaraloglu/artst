...
import { FADE_IN_ANIMATION_SETTINGS, HOME_DOMAIN } from "@artst/utils";
		...
		<div className="relative grid gap-5 border-b border-gray-200 pb-5">
			<div className="flex items-center justify-between">
				<div className="flex items-center justify-between space-x-2">
					<h2 className="text-sm font-medium text-gray-900">Custom Social Media Cards</h2>
					<InfoTooltip
						content={
							<SimpleTooltipContent
				...
				<motion.div key="og-options" {...FADE_IN_ANIMATION_SETTINGS} className="grid gap-5">
					<div>
							...
							{fileError ? (
								...
								<div className="flex items-center justify-between">
									<button
										...
										type="button"
										onClick={() => {
											const image = window.prompt("Paste a URL to an image (max 5MB)", "https://");
											if (image) {
									...
									<Popover
										content={<UnsplashSearch setData={setData} setOpenPopover={handleSet} />}
						...
						>
							...
							{image && (
								<img src={image} alt="Preview" className="h-full w-full rounded-md object-cover" />
							)}
						</label>
					...
					<div>
						...
						<div className="relative mt-1 flex rounded-md shadow-sm">
							...
							<TextareaAutosize
								...
								placeholder={`${process.env.NEXT_PUBLIC_APP_NAME} - artist management infrastructure.`}
								value={title || ""}
								onChange={(e) => {
					...
					<div>
						...
						<div className="relative mt-1 flex rounded-md shadow-sm">
							...
							<TextareaAutosize
								...
								maxLength={240}
								className="block w-full rounded-md border-gray-300 text-gray-900 placeholder-gray-300 focus:border-gray-500 focus:outline-none focus:ring-gray-500 sm:text-sm"
								placeholder={`${process.env.NEXT_PUBLIC_APP_NAME} is artist management infrastructure for modern marketing teams.`}
								value={description || ""}
								onChange={(e) => {
									setData({
										...data,
