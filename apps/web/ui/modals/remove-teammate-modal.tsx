...
import { Avatar, Button, Logo, Modal } from "@artst/ui";
			...
			<div className="flex flex-col space-y-4 bg-gray-50 px-4 py-8 text-left sm:px-16">
				...
				<Button
					...
					onClick={() => {
						setRemoving(true);
						fetch(
							`/api/projects/${slug}/${invite ? `invites?email=${email}` : `users?userId=${id}`}`,
							{
								method: "DELETE",
							...
							if (res.status === 200) {
								await mutate(`/api/projects/${slug}/${invite ? "invites" : "users"}`);
