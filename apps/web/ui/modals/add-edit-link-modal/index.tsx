...
function AddEditLinkModal({
	...
	const { allActiveDomains: domains, primaryDomain, defaultDomains } = useDomains();
	...
	useEffect(() => {
		...
		if (
			...
			fetch(`/api/links/exists?domain=${domain}&key=${debouncedKey}&projectSlug=${slug}`).then(
				async (res) => {
					if (res.status === 200) {
								...
								<div className="flex items-center justify-between">
									...
									{props && lockKey ? (
										...
										<button
											...
											type="button"
										>
											{generatingKey ? <LoadingCircle /> : <Random className="h-3 w-3" />}
