...
import { Badge, Button, IconMenu, Logo, Modal, Popover, Tick, useRouterStuff } from "@artst/ui";
import {
	HOME_DOMAIN,
...
function UpgradePlanModal({
	...
	const currentPlan =
		SELF_SERVE_PAID_PLANS.find((p) => p.name.toLowerCase() === plan) ?? SELF_SERVE_PAID_PLANS[0];
	...
	return (
		...
		>
			...
			>
				...
				<motion.p className="text-center text-sm text-gray-500" variants={STAGGER_CHILD_VARIANTS}>
					Enjoy higher limits and extra features with artst.io {currentPlan.name}
				</motion.p>
			</motion.div>
							...
							<div className="w-full p-2 md:w-56">
								{SELF_SERVE_PAID_PLANS.map(({ name, colors }, i) => (
									...
									>
										<IconMenu
											text={name}
											icon={<div className={cn("h-2 w-2 rounded-full", colors.bg)} />}
										/>
										{plan === name.toLowerCase() && <Tick className="h-4 w-4" aria-hidden="true" />}
									</button>
								))}
							</div>
						}
					...
					>
						...
						>
							<IconMenu
								...
								icon={<div className={cn("h-2 w-2 rounded-full", currentPlan.colors.bg)} />}
							/>
							<ChevronDown
							...
							<div className="w-full p-2 md:w-36">
								{PERIODS.map((p, i) => (
									<button
										...
										className="flex w-full items-center justify-between space-x-2 rounded-md p-2 hover:bg-gray-100 active:bg-gray-200"
									>
										<span className="text-sm text-gray-500">{capitalize(p)}</span>
										{period === p && <Tick className="h-4 w-4" aria-hidden="true" />}
									</button>
								))}
							</div>
						}
				...
				>
					<div className="flex items-center justify-between">
						<div className="flex items-center space-x-2">
							...
							<Badge variant="neutral" className="text-sm font-normal normal-case">
								${currentPlan.price[period].toString()}
								/mo
								<span className="hidden sm:inline-block">, billed {period}</span>
							</Badge>
						</div>
					</div>
					<motion.div
					...
					<Button
						...
						onClick={() => {
							setClicked(true);
							fetch(`/api/projects/${slug}/billing/upgrade?plan=${plan}_${period}`, {
								method: "POST",
							})
								.then(async (res) => {
