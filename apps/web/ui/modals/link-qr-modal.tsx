...
export function QRCodePicker({
	...
	const qrLogoUrl = useMemo(() => {
		...
		return typeof window !== "undefined" && window.location.origin
			...
			: "https://artst.io/_static/logo.svg";
	}, [logo, plan]);
		...
		<>
			...
			<div className="flex flex-col space-y-6 bg-gray-50 py-6 text-left sm:rounded-b-2xl">
				...
				<div className="grid grid-cols-2 gap-2 px-4 sm:px-16">
					...
					<QrDropdown download={download} qrData={qrData} showLogo={showLogo} logo={qrLogoUrl} />
				</div>
...
function AdvancedSettings({ qrData, setFgColor, showLogo, setShowLogo, setShowLinkQRModal }) {
						...
						{plan !== "free" ? (
							<div className="mt-1 flex items-center space-x-2">
								...
								<p className="text-sm text-gray-600">
									Show {!slug || (!logo && process.env.NEXT_PUBLIC_APP_NAME)} Logo
								</p>
							</div>
						) : (
							<Tooltip
							...
							download(
								getQRAsSVGDataUri({
									...
									...(showLogo && {
										imageSettings: {
											...qrData.imageSettings,
											src: logo || "https://artst.io/_static/logo.svg",
										},
									}),
								}),
								"svg",
