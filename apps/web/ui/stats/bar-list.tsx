...
import { NumberTooltip, Tooltip } from "@artst/ui";
import { cn, nFormatter } from "@artst/utils";
	...
	const bars = (
		<div className="grid gap-4">
			{filteredData.map(({ icon, title, href, clicks }, idx) => {
				...
				return (
					<Link key={idx} href={href} scroll={false} onClick={() => setShowModal(false)}>
						<div key={idx} className="group flex items-center justify-between">
							<div className="relative z-10 flex w-full max-w-[calc(100%-2rem)] items-center">
								{tab === "Top Links" ? (
									<Tooltip content={<LinkPreviewTooltip link={title} />}>{lineItem}</Tooltip>
								) : (
									lineItem
								)}
								<motion.div
									...
									className={cn("absolute h-8 origin-left rounded-sm", barBackground)}
