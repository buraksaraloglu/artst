			...
			<div className="mx-auto flex h-20 max-w-4xl flex-col items-center justify-between space-y-3 px-2.5 md:h-10 md:flex-row md:space-y-0 lg:px-0">
				{isPublicStatsPage ? (
					...
					<div className="flex items-center space-x-2">
						<BlurImage
							...
							height={48}
						/>
						<h2 className="text-lg font-semibold text-gray-800">{primaryDomain}</h2>
						{allProjectDomains && allProjectDomains.length > 1 && (
							<Tooltip content={<DomainsFilterTooltip domains={allProjectDomains} />} side="bottom">
				...
				<div className="flex items-center">
						...
						content={
							<div className="grid w-full p-2 md:w-48">
								{INTERVALS.map(({ display, value }) =>
									(value === "all" || value === "90d") && (!plan || plan === "free") ? (
										<Tooltip
											key={value}
											content={
												<TooltipContent
					...
					>
						...
						>
							<IconMenu text={selectedInterval.display} icon={<Calendar className="h-4 w-4" />} />
							<ChevronDown
			...
			<div className="flex w-full flex-col">
				{domains.map(({ slug, target }) => (
					...
					>
						<div className="flex items-center space-x-2">
							<BlurImage
								src={`${GOOGLE_FAVICON_URL}${target ? getApexDomain(target) : SHORT_DOMAIN}`}
								alt={slug}
								className="h-5 w-5 rounded-full"
								width={20}
								height={20}
							/>
							<p className="w-32 truncate text-left text-sm font-semibold text-gray-500">{slug}</p>
						</div>
...
const SharePopover = () => {
	...
	const { baseApiPath, queryString, domain, key } = useContext(StatsContext) as {
		baseApiPath: string;
		queryString: string;
		domain: string;
	...
	const { data } = useSWR<{ publicStats: boolean }>(`${baseApiPath}?${queryString}`, fetcher);
				...
				<div className="w-full divide-y divide-gray-200 text-sm md:w-60">
					...
					<div className="p-4">
						<div className="mb-2 flex items-center justify-between">
							...
							<Switch checked={data?.publicStats} fn={handleChange} disabled={updating} />
						</div>
						<p className="text-gray-500">
							Making stats public will allow anyone with the link to see the stats for this short
							link.
						</p>
					</div>
					<div className="p-4">
