...
import { Button, LoadingCircle, LoadingDots, NumberTooltip } from "@artst/ui";
import { capitalize, fetcher, nFormatter, truncate } from "@artst/utils";
			...
			<div className="flex flex-col space-y-3 rounded-lg border border-gray-200 bg-white px-5 py-8 sm:px-10">
					...
					<div className="flex items-center space-x-2">
						...
						<NumberTooltip value={clicks}>
							...
							>
								...
								<p className="text-sm">
									{!clicks && clicks !== 0 ? <LoadingDots /> : nFormatter(clicks)}
				...
				<div className="flex h-10 flex-col space-y-2 sm:flex-row sm:items-center sm:space-x-5 sm:space-y-0">
					<div className="flex items-center space-x-2">
						...
						<p className="text-sm text-gray-500">{data ? data.status : "Checking Domain Status"}</p>
					</div>
					<div className="flex items-center space-x-2">
						{target ? (
							<CheckCircleFill className="h-6 w-6 text-blue-500" />
						...
						<div className="flex space-x-1">
							...
							{target && (
								...
								>
									{truncate(target.replace(/^(?:https?:\/\/)?(?:www\.)?/i, ""), 24)}
								</a>
							)}
						</div>
					</div>
				</div>
				{data && data.status !== "Valid Configuration" && <DomainConfiguration data={data} />}
			</div>
		</>
	);
}
