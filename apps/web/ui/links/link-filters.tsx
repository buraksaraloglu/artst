...
import { SWIPE_REVEAL_ANIMATION_SETTINGS, nFormatter, truncate } from "@artst/utils";
...
export default function LinkFilters() {
	...
	const showClearButton = useMemo(() => {
		return ["sort", "search", "domain", "userId", "tagId", "showArchived", "page"].some(
						...
						{options?.map(({ value, count }) => (
							...
							>
								<input
									id={value}
									name={value}
									checked={searchParams?.get("domain") === value || domains?.length <= 1}
									onChange={() => {
										queryParams({
											set: {
												domain: value,
			...
			content={
				<div className="flex w-48 flex-col divide-y divide-gray-200">
					<div className="p-2">
						...
						>
							<div className="my-2 flex items-center justify-between px-3">
								...
								{data !== tag && <button className="text-xs text-gray-500">Save</button>}
							</div>
							<input
								type="text"
								autoFocus
					...
					<div className="p-2">
						...
						>
							<IconMenu text="Delete Tag" icon={<Trash className="h-4 w-4 text-red-600" />} />
						</button>
					</div>
				</div>
			}
...
const MyLinksFilter = () => {
	...
	return (
		<div className="flex items-center justify-between py-6">
			<label className="text-sm font-medium text-gray-600">Show my links only</label>
			<Switch
				fn={() =>
					queryParams(
						userId
...
const ArchiveFilter = () => {
	...
	return (
		<div className="flex items-center justify-between py-6">
			<label className="text-sm font-medium text-gray-600">Include archived links</label>
			<Switch
				fn={() =>
					queryParams(
						showArchived
