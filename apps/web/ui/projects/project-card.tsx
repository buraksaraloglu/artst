...
export default function ProjectCard({
	...
	const { data: user } = useSWRImmutable<Session["user"] & { migratedProject: string | null }>(
		`/api/user`,
		fetcher,
	);
			...
			{isMigratedProject && (
				<>
					...
					<div className="absolute -bottom-28 right-0 z-10 w-full max-w-[16rem] rounded-lg border border-gray-200 bg-white p-3 text-center text-sm shadow lg:-right-56">
						<p>
							Your <InlineSnippet>artst.io</InlineSnippet> links have been migrated to a custom
							project.
						</p>
						<a
