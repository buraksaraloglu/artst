		...
		<form
			onSubmit={async (e) => {
				...
				fetch(`/api/projects/${slug}/logo`, {
					...
					if (res.status === 200) {
						await Promise.all([mutate("/api/projects"), mutate(`/api/projects/${slug}`)]);
		...
		>
			<div className="flex flex-col space-y-3 p-5 sm:p-10">
				...
				<div>
					...
					>
						...
						{image && (
							<img src={image} alt="Preview" className="h-full w-full rounded-full object-cover" />
						)}
					</label>
			...
			<div className="flex items-center justify-between space-x-4 rounded-b-lg border-t border-gray-200 bg-gray-50 p-3 sm:px-10">
				...
				<div className="shrink-0">
					<Button text="Save changes" loading={uploading} disabled={!image || logo === image} />
				</div>
			</div>
		</form>
	);
