import { LOCALHOST_GEO_DATA, LOCALHOST_IP, capitalize, getDomainWithoutWWW } from "@artst/utils";
 ...
 * If key is not specified, record click as the root click ("_root", e.g. artst.io, vercel.fyi)
 **/
export async function recordClick({
	...
	const { success } = await ratelimit(2, "1 h").limit(
		`recordClick:${ip}:${domain.toLowerCase()}:${key?.toLowerCase() || "_root"}`,
	);
	if (!success) {
		return null;
	}
	...
	return await Promise.allSettled([
		...
		key
			...
			: conn.execute("UPDATE Domain SET clicks = clicks + 1, lastClicked = NOW() WHERE slug = ?", [
					domain,
			  ]),
	]);
}
...
export async function deleteClickData({ domain, key }: { domain: string; key: string }) {
	if (!domain || !key) {
		return null;
	}
