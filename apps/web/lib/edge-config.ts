...
export const isBlacklistedDomain = async (domain: string) => {
	...
	try {
		[blacklistedDomains, blacklistedTerms] = await Promise.all([get("domains"), get("terms")]);
	} catch (e) {
		blacklistedDomains = [];
		blacklistedTerms = [];
	}
	...
	return (
		(blacklistedDomains && blacklistedDomains.includes(domainToTest)) ||
		(blacklistedTerms && new RegExp(blacklistedTerms.join("|")).test(domainToTest))
	);
};
...
export const isBlacklistedKey = async (key: string) => {
	...
	return (
		...
		new RegExp(blacklistedKeys.join("|"), "i").test(key)
	);
};
