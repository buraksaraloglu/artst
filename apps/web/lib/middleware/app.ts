...
export default async function AppMiddleware(req: NextRequest) {
	...
	if (!session?.email && path !== "/login" && path !== "/register" && path !== "/auth/saml") {
		...
		if (
			...
			const [existingInvite, existingProject] = [
				await conn
					?.execute("SELECT projectId FROM ProjectInvite WHERE email = ?", [session.email])
					.then((res) => res.rows[0] as { projectId: string } | undefined),
				await conn
					?.execute("SELECT projectId FROM ProjectUsers WHERE userId = ?", [session.user.id])
					.then((res) => res.rows[0] as { projectId: string } | undefined),
			];
			...
			if (existingInvite || existingProject) {
				...
				const project = await conn
					?.execute("SELECT slug from Project WHERE id = ?", [
						existingInvite ? existingInvite.projectId : existingProject?.projectId,
					])
	...
	return NextResponse.rewrite(new URL(`/app.artst.io${fullPath === "/" ? "" : fullPath}`, req.url));
}
