...
export default async function LinkMiddleware(req: NextRequest, ev: NextFetchEvent) {
	...
	if (process.env.NODE_ENV !== "development" && domain === "artst.io" && key === "try") {
		if (await isBlacklistedReferrer(req.headers.get("referer"))) {
			return new Response("Don't DDoS me pls 🥺", { status: 429 });
		}
		const ip = ipAddress(req) || LOCALHOST_IP;
		const { success } = await ratelimit(10, "1 d").limit(`${ip}:${domain}:${key}`);
	...
	if (target) {
		...
		if (banned) {
			return NextResponse.rewrite(new URL(`/banned/${domain}/${encodeURIComponent(key)}`, req.url));
		}
		...
		if (isBot && proxy) {
			return NextResponse.rewrite(new URL(`/proxy/${domain}/${encodeURIComponent(key)}`, req.url));

			// rewrite to target URL if link cloaking is enabled
		} else if (rewrite) {
			if (iframeable) {
				return NextResponse.rewrite(new URL(`/rewrite/${target}`, req.url), ARTST_HEADERS);
			} else {
			...
			return NextResponse.redirect(getFinalUrl(android, { req }), ARTST_HEADERS);

			// redirect to geo-specific link if it is specified and the user is in the specified country
		} else if (geo && country && country in geo) {
			return NextResponse.redirect(getFinalUrl(geo[country], { req }), ARTST_HEADERS);
