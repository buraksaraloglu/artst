...
import { ReleaseProps, ProjectProps } from "../types";

export async function getReleasesForProject({
	projectId,
	search,
	sort = "createdAt",
	page,
...
export async function processRelease({
	...
	payload: ReleaseProps;
	project?: ProjectProps;
	session?: Session;
	bulk?: boolean;
}) {
	let { id, title, releaseDate, releaseAudio, projectId, coverImage } = payload;
	...
	const uploadedImage = coverImage && coverImage.startsWith("data:image") ? true : false;
...
export async function addRelease(release: ReleaseProps) {
	const { coverImage, title } = release;
	...
	let [response] = await Promise.all([
		prisma.release.create({
			data: {
				...
				coverImage: uploadedImage ? undefined : coverImage,
			},
		}),
		// 	redis.set(
		// 		`${release.projectId}:${}`,
	...
	if (coverImage) {
		const { secure_url } = await cloudinary.v2.uploader.upload(coverImage, {
			// public_id: key,
			// folder: release.projectId,
			overwrite: true,
			invalidate: true,
		});
		response = await prisma.release.update({
			where: {
				id: response.id,
			},
			data: {
...
// export async function bulkCreateLinks(links: ReleaseProps[]) {
