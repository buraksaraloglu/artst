...
import { getApexDomain, getDomainWithoutWWW, validDomainRegex } from "@artst/utils";
...
export const validateDomain = async (domain: string) => {
	...
	const validDomain =
		...
		// make sure the domain doesn't contain artst.io/artst.io
		!/^(artst\.co|.*\.artst\.co|artst\.sh|.*\.artst\.sh)$/i.test(domain);
...
export async function changeDomainForImages(domain: string, newDomain: string) {
	...
	try {
		return await Promise.all(
			links.map(({ key }) =>
				cloudinary.v2.uploader.rename(`${domain}/${key}`, `${newDomain}/${key}`, {
					invalidate: true,
				}),
			),
		);
