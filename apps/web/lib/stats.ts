...
export const VALID_STATS_FILTERS = ["country", "city", "device", "browser", "os", "referer"];
...
export const getStats = async ({
	...
	if (endpoint === "clicks" && key && !interval) {
		const response =
			key === "_root"
				? await conn.execute("SELECT clicks FROM Domain WHERE slug = ?", [domain])
				: await conn.execute("SELECT clicks FROM Link WHERE domain = ? AND `key` = ?", [
						domain,
	...
	if (interval) {
		url.searchParams.append(
			"start",
			intervalData[interval].startDate.toISOString().replace("T", " ").replace("Z", ""),
		);
		url.searchParams.append(
			"end",
