...
export default function useDomains({ domain }: { domain?: string } = {}) {
	...
	const allDomains = [...(data || []), ...(slug !== "artst" ? defaultDomains : [])];
	const allActiveDomains = [...(projectDomains || []), ...(slug !== "artst" ? defaultDomains : [])];
	...
	const verified = domain
		? // If a domain is passed, check if it's verified
		  [...allActiveDomains, ...(archivedProjectDomains || [])].find((d) => d.slug === domain)
				?.verified
